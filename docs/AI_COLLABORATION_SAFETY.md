...
This layer does not assume that an AI assistant will be error-free. Instead, controls are proportional to risk: work should be fast by default and strict when risky, while high-impact work fails closed unless the required evidence and checks are present.
...
16. APPROVAL_GATE_BYPASS_OR_MISMATCH
    - using a permission/workflow mode that does not match the project's risk-based approval model;
    - stretching approval for one bounded change-set to a different target, expanded scope, materially different fallback, merge, deploy, destructive action, or production action.
...
The depth of this loop is proportional to risk. `docs/WORK_PROTOCOL.md` defines LOW, MEDIUM, and HIGH.

### 1. Risk classification
Classify the objective before acting.

- LOW uses minimal fresh authority verification and proportional validation.
- MEDIUM uses explicit scope/preflight, a task branch, tests/validation, remote diff, and PR/review.
- HIGH requires fresh remote/live verification and DEEP AUDIT.

HIGH includes Architecture or governance changes, merge decisions for high-impact work, production, deploys, QC milestone decisions, scientific-method/data-selection decisions, policy/guardrail changes, destructive operations, recovery/cleanup, security/permission changes, or stale/confused/uncertain state.

### 2. Authority loading
Load fresh project authority appropriate to the risk and task:
- current remote GitHub branch/main state when repository state matters;
- current Architecture v1 control files relevant to the task;
...
### 3. Constraint ledger
Before a HIGH change, and for MEDIUM work with multiple material constraints, maintain an explicit mental or written checklist of all user constraints and task requirements that could materially affect correctness.
...
Use the GitHub task branch as the default repository write target for MEDIUM/HIGH changes. LOW changes may use the smallest safe repository workflow consistent with repository protections and the declared scope.

If a local-first exception is necessary, state the technical reason and synchronization path before changing files.

Do not silently modify shared scientific policy from a centre/metric workstream.

### 5A. Work protocol and scope boundary
Every task must declare exactly one primary deliverable before execution. The deliverable must be concrete, bounded, and verifiable. Work outside it is out of scope unless the human explicitly approves it.

Use `docs/WORK_PROTOCOL.md` for the task header. It distinguishes `INSPECT` from `CHANGE` and records risk level.
...
A `CHANGE` task may begin only after the preconditions, exact allowed file/system scope, intended change, validation plan, and required human approval for that bounded change-set are clear.

### 5B. GitHub permission and approval model
Connected GitHub should be configured to support **Fast by default, strict when risky**: allow low-risk actions where the platform can classify them safely, while sensitive operations remain reviewable/blockable.

One explicit human approval may cover one predeclared bounded change-set when the target branch, file/system scope, intended changes, and validation are already stated. While scope and method remain unchanged, that approval may cover in-scope branch writes, commits, validation, and PR preparation/creation.

Fresh approval is required if the scope expands, the target changes, or the fallback/mutation method changes materially.

The following always require their own independent human approval immediately before the operation:
- merge;
- deploy;
- delete/destructive operations;
- production writes/downloads.

Approval of implementation never implies approval of those sensitive operations.

If a write is blocked or rejected, re-read the remote system of record before retrying. Use only the least-risk fallback consistent with project governance, and obtain fresh approval if the fallback materially changes the workflow or mutation scope.

### 5C. Assistant execution responsibility
When project tools are available, ChatGPT/Codex is responsible for performing repository inspection, branch work, bounded edits, diffs, applicable tests/validation, and PR preparation/creation. Do not transfer routine executable work to the human merely for assistant convenience.

Human involvement is reserved for decisions, required approvals, credentials/access, or actions the available tools genuinely cannot perform.

### 5D. ADR rule
Architecture changes require an Architecture Decision Record before adoption. ADRs are not required for typo fixes, ordinary documentation maintenance, or centre-specific evidence collection.

Scientific decisions continue to use `docs/SCIENTIFIC_DECISION_TRACEABILITY.md`; an ADR does not replace the scientific-decision process.

### 6. Post-action verification
After every repository-changing action that matters to project state, re-read or re-query the resulting remote state rather than relying on the assistant's memory of what it attempted to write.

For HIGH repository work, verify as applicable:
...
For LOW/MEDIUM work, use the smallest post-action checks sufficient to prove the claimed result.
...
HIGH completion must not rely only on the assistant reviewing its own prose or remembering its own actions.
...
Before declaring HIGH work complete, perform a second pass that asks:
...
For HIGH work this second pass is mandatory and must be evidence-based, not merely a confidence statement.
...
Before using a tool for a HIGH write workflow, confirm that the tool actually supports the required operation. Tool failure or safety blocking must not be silently bypassed by switching to a riskier workflow.
...
HIGH changes require two logically separate passes:
...
Prevention: risk classification now precedes workflow selection; HIGH invokes Deep Audit.
Status: ENCODED.
...
Prevention: HIGH re-entry begins with fresh remote GitHub branch/main comparison; local packs are non-authoritative; the defective v2 generator was removed from the active tree and preserved only in Git history.
Status: ENCODED.
...
Prevention: this AI Collaboration Safety Layer, mandatory control loop, defense-in-depth verification, negative-claim rule, two-pass HIGH review, no-silent-omission rule, completion gate, and incident-to-guardrail promotion.
Status: ENCODED AND INTEGRATED INTO CURRENT ARCHITECTURE CONTROLS.

### WF-006 — GitHub write approval mismatch
Failure: GitHub's earlier permission mode caused repeated write-approval/safety blocking and unnecessary fallback attempts, leaving candidate changes stranded off `main` and wasting user time.
Root cause: the connector permission mode and the project's desired risk-based workflow were misaligned.
Impact/risk: routine bounded writes were repeatedly blocked while sensitive-operation approval semantics remained unclear.
Immediate correction: GitHub was configured to `Allow low-risk actions` / `review_important_actions`, matching the project's risk-based model.
Prevention: bounded LOW/MEDIUM actions may proceed under their declared scope and approval model; merge, deploy, destructive actions, and production actions retain independent approval gates; blocked/ambiguous writes still require remote state re-read before retry.
Status: ENCODED IN WORKFLOW-HARDENING CANDIDATE; requires approved merge before becoming `main` truth.

## Unknown future failure modes
The absence of a known incident does not imply safety.

If a new assistant behavior appears capable of corrupting scientific truth, repository state, user data, reproducibility, or milestone decisions, the default is fail closed for the affected HIGH action until the behavior is understood and a preventive control is recorded.
